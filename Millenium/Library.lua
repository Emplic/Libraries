        ...
        function library:window(properties)
            ...
            local items = cfg.items; do
                ...
                items[ "other_info" ] = library:create( "TextLabel" , {
                    ...
                    TextColor3 = themes.preset.accent;
                    BorderColor3 = rgb(0, 0, 0);
                    Text = '<font color="rgb(72, 72, 73)">freemium, </font>' .. cfg.name .. cfg.suffix;
                    Size = dim2(1, 0, 0, 0);
                    Position = dim2(0, -10, 0.5, -1);
                    AnchorPoint = vec2(0, 0.5);
                    BorderSizePixel = 0;
...
function library:init_config(window) 
            ...
            local function get_cfg_name()
                ...
                local list = flags["config_name_list"]
                return (text ~= "" and text) or list
            end
            ...
            section:button({
                name = "Load", 
                callback = function() 
                    local name = flags["config_name_list"] -- Always load from list selection
                    if not name or name == "" then return end
                    ...
                    if isfile(path) then
                        ...
                        notifications:create_notification({name = "Configs", info = "Error: Config not found"}) 
                    end
                end
            })
